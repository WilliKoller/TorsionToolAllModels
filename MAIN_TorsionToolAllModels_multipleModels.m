...
% %Method after Hernandez - measured by Willi
% % default_Anteversion = 13;
% % % Perimeter Method from Geometry
% % default_Anteversion = 16;
% % default_NeckShaftAngle = 127;
...
matFile = 'C:\Users\willi\ucloud\PhD\Study_GrowthValidation\Matlab\femurAnglesSangeux_byWilli_checkedWithLeon.mat';
load(matFile);

participants = fieldnames(femurAngles);

Set_AVA = [-12, 3, 18, 33, 48];
Set_NSA = [93, 108, 123, 138, 153];
...
for p = 1 : numel(participants)
    ...
    for session = {'S1', 'S2'}
        ...
        AVA_inPlanePerpendicularToShaft_r = femurAngles.(participant).(session{1}).R.AVA;
        AVA_inPlanePerpendicularToShaft_l = femurAngles.(participant).(session{1}).L.AVA;
        NSA_r = femurAngles.(participant).(session{1}).R.NSA;
        NSA_l = femurAngles.(participant).(session{1}).L.NSA;
        TT_r = default_TibiaTorsion;
        TT_l = default_TibiaTorsion;
        ...
        deform_bone = 'F';
        which_leg = 'R';
        angle_AV_right = AVA_inPlanePerpendicularToShaft_r; % right anteversion angle (in degrees) %
        angle_NS_right = NSA_r; % right neck-shaft angle (in degrees) %
        ...
        deform_bone = 'F';
        which_leg = 'L';
        angle_AV_left = AVA_inPlanePerpendicularToShaft_l; % left anteversion angle (in degrees) %
        angle_NS_left = NSA_l; % left neck-shaft angle (in degrees) %
        ...
        deform_bone = 'T';
        which_leg = 'R';
        angle_TT_right = TT_r; % right tibial torsion angle (in degrees) %
        ...
        deform_bone = 'T';
        which_leg = 'L';
        angle_TT_left = TT_l; % left tibial torsion angle (in degrees) %
        ...
        %% rename file
        copyfile('./DEFORMED_MODEL/FINAL_PERSONALISEDTORSIONS.osim', ['./DEFORMED_MODEL/' participant '_' session{1} '_LNSA' num2str(NSA_l, 3) '_LAVA' num2str(AVA_inPlanePerpendicularToShaft_l, 3) ...
            '_LTT' num2str(TT_l, 3) '_RNSA' num2str(NSA_r, 3) '_RAVA' num2str(AVA_inPlanePerpendicularToShaft_r, 3) '_RTT' num2str(TT_r, 3) '.osim']);
        delete('./DEFORMED_MODEL/FINAL_PERSONALISEDTORSIONS.osim');
        copyfile('./DEFORMED_MODEL', ['C:\Users\willi\ucloud\PhD\Study_LongitudinalMSK\Models_Sangeux\' participant '_' session{1}]);
    end
end